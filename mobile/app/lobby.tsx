...
import { LanguageChoiceGroup } from "../components/LanguageChoiceGroup";
...
import { inviteSharingAccessibility } from "../lib/inviteSharingAccessibility";
...
export default function LobbyScreen() {
  ...
  const start = async () => {
    if (meetingPrivacyDefaults.requiresExplicitConsent && !consent) { Alert.alert("موافقة مطلوبة", lobbyReadinessCopy.consent.required); return; }
    try {
      setCreating(true);
  ...
  const testMicrophone = async () => {
    try {
      ...
      setTested(true);
      Alert.alert("الميكروفون جاهز", lobbyReadinessCopy.microphone.ready);
    } catch { Alert.alert("تعذر الاختبار", "تأكد من إعدادات الميكروفون في الجهاز ثم حاول مرة أخرى."); }
  };
  return <Screen><ScrollView contentContainerStyle={styles.content}><View style={styles.top}><GhostButton title="الرئيسية" onPress={() => router.back()} /><Brand /></View><View style={styles.step}><Text style={styles.stepText}>الخطوة 1 من 2</Text></View><Text style={styles.title}>اختر كيف تريد أن تتحدث وتفهم</Text><Text style={styles.body}>يظل النص الأصلي بجوار ترجمته، ويمكنك تعديل إعداداتك قبل الانضمام.</Text><View style={styles.card}><Text style={styles.label}>اسم الاجتماع</Text><TextInput value={title} onChangeText={setTitle} style={styles.input} textAlign="right" maxLength={160} /><LanguageChoiceGroup label="لغة الحديث" value={speaking} choices={supportedLanguages} onChange={setSpeaking} /><LanguageChoiceGroup label="لغة العرض والاستماع" value={display} choices={supportedLanguages} onChange={setDisplay} /><View style={styles.divider} /><View style={styles.audio}><View><Text style={styles.audioTitle}>{lobbyReadinessCopy.microphone.title}</Text><Text style={styles.audioHint}>{tested ? lobbyReadinessCopy.microphone.ready : lobbyReadinessCopy.microphone.untested}</Text></View><View style={[styles.status, tested && styles.statusOk]}><Text style={styles.statusText}>{tested ? "جاهز" : "غير مختبر"}</Text></View></View><GhostButton title={lobbyReadinessCopy.microphone.testAction} onPress={() => void testMicrophone()} /><View style={styles.audio}><View><Text style={styles.audioTitle}>{lobbyReadinessCopy.invite.title}</Text><Text style={styles.audioHint}>{getLobbyInviteGuidance(inviteEnabled)}</Text></View><Switch value={inviteEnabled} onValueChange={setInviteEnabled} accessibilityRole="switch" accessibilityLabel={inviteSharingAccessibility.label} accessibilityState={inviteSharingAccessibility.state(inviteEnabled)} trackColor={{ false: "#CBD5E1", true: colors.indigo }} /></View><View style={styles.consent}><View style={styles.consentCopy}><Text style={styles.consentTitle}>{lobbyReadinessCopy.consent.title}</Text><Text style={styles.consentHint}>{meetingPrivacyDefaults.requiresAllActiveParticipants ? lobbyReadinessCopy.consent.detail : "لن يُحفظ المحتوى ما لم توافق أنت والمشاركون الحاضرون."}</Text></View><Switch value={consent} onValueChange={setConsent} trackColor={{ false: "#CBD5E1", true: colors.indigo }} /></View><PrimaryButton title={creating ? "جارٍ إنشاء الاجتماع…" : "إنشاء الاجتماع والدخول"} onPress={() => void start()} /></View></ScrollView></Screen>;
}
