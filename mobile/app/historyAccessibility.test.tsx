...
const mocks = vi.hoisted(() => ({ getNativeMeetingHistory: vi.fn() }));
...
describe("HistoryScreen empty state accessibility", () => {
  beforeEach(() => { mocks.getNativeMeetingHistory.mockReset(); });
  ...
  it("renders server-backed meeting cards after loading", async () => {
    ...
    let renderer: ReturnType<typeof create>;
    await act(async () => { renderer = create(<HistoryScreen />); await Promise.resolve(); });
    const cards = renderer!.root.findAll(node => String(node.type) === "View" && node.props.accessibilityLabel === "جلسة اختبار. نشطة.");
