...
import { supportedLanguages } from "../lib/specs";
import { colors } from "../lib/theme";

const speedChoices = [0.8, 1, 1.2];
...
export default function SettingsScreen() {
  ...
  useEffect(() => { void loadMobilePreferences().then(preferences => { setSpeechLanguage(preferences.speechLanguage); setDisplayLanguage(preferences.displayLanguage); setVoiceRate(preferences.voiceRate); setStorage(preferences.askBeforeStorage); setReminders(preferences.meetingReminders); setReady(true); }); }, []);

  const save = async () => {
    try {
      await saveMobilePreferences({ speechLanguage, displayLanguage, voiceRate, askBeforeStorage: storage, meetingReminders: reminders });
