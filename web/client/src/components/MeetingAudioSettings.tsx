...
export default function MeetingAudioSettings({
  ...
  return <section aria-label="إعدادات الصوت واللغة" className="mt-4 overflow-hidden rounded-[22px] border border-indigo-100 bg-white shadow-[0_18px_40px_-32px_rgba(44,49,94,.45)]">
    ...
    <div className="border-t border-slate-100 p-4">
      ...
      <div className="mt-4 rounded-2xl bg-slate-50 p-3"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-xs font-bold text-slate-700"><Volume2 className="h-3.5 w-3.5 text-indigo-600" />مستوى الإدخال</span><span className="text-[11px] font-bold text-slate-500">{testing ? `${level}%` : "غير نشط"}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-gradient-to-l from-emerald-400 via-indigo-500 to-indigo-600 transition-[width] duration-100" style={{ width: `${testing ? Math.max(3, level) : 0}%` }} /></div></div>
      <p className={`mt-3 flex gap-2 rounded-xl px-3 py-2 text-[11px] leading-5 ${status === "ready" ? "bg-emerald-50 text-emerald-700" : status === "idle" ? "bg-indigo-50 text-indigo-700" : "bg-amber-50 text-amber-800"}`}>{status === "ready" ? <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0" /> : <CircleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0" />}{statusMessage}</p>
    </div>
  </section>;
}
