...
import { getLiveTranscriptStatus, getMuteControlLabel, liveTranscriptCopy, meetingControlCopy } from "@linguabridge/contracts/meetingSpecs";
...
function ControlButton({ label, active, pressed, onClick, children }: { label: string; active?: boolean; pressed?: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} aria-label={label} aria-pressed={pressed} className={`grid h-10 w-10 place-items-center rounded-xl border transition-colors ${active ? "border-indigo-200 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}>{children}</button>;
}
...
export default function Meeting() {
  ...
  const [pendingLiveTranslations, setPendingLiveTranslations] = useState(0);
  ...
  const handleRecognizedText = useCallback(async (text: string) => {
    if (!text) return;
    setPendingLiveTranslations(value => value + 1);
    try {
      ...
      setPendingLiveTranslations(value => Math.max(0, value - 1));
    }
  ...
  return <div className="min-h-screen bg-[#f6f7fb]" dir="rtl">
    ...
    <footer className="sticky bottom-0 z-30 border-t border-slate-200/85 bg-white/95 px-3 py-3 backdrop-blur-xl"><div className="mx-auto flex max-w-[760px] items-center justify-between gap-2"><div className="flex gap-2"><ControlButton label={getMuteControlLabel(muted)} active={!muted} pressed={muted} onClick={() => setMuted(value => !value)}>{muted ? <MicOff className="h-4.5 w-4.5" /> : <Mic className="h-4.5 w-4.5" />}</ControlButton><ControlButton label={meetingControlCopy.audio.deviceTest} onClick={() => toast.info("اختبر الميكروفون من الردهة أو اسمح للمتصفح بالوصول إلى الجهاز.")}><Headphones className="h-4.5 w-4.5" /></ControlButton><ControlButton label={recording ? "إيقاف النص الحي" : "تشغيل النص الحي"} active={recording} pressed={recording} onClick={toggleLiveSpeech}>{recording ? <Captions className="h-4.5 w-4.5" /> : <Pause className="h-4.5 w-4.5" />}</ControlButton></div><div className="hidden text-center sm:block"><p className="text-xs font-bold text-slate-700">00:18:42</p><p className="mt-0.5 text-[10px] text-slate-400">{privacy.data?.persistenceAllowed ? "الحفظ مفعل بموافقة الحاضرين" : "الحفظ بانتظار موافقة الحاضرين"}</p></div><div className="flex gap-2"><ControlButton label={speaking ? "إيقاف قراءة الترجمة" : "قراءة آخر ترجمة"} active={speaking} pressed={speaking} onClick={speakLatestTranslation}>{speaking ? <VolumeX className="h-4.5 w-4.5" /> : <Volume2 className="h-4.5 w-4.5" />}</ControlButton><span className={`hidden items-center gap-1.5 rounded-xl px-3 text-xs font-bold sm:flex ${privacy.data?.persistenceAllowed ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{privacy.data?.persistenceAllowed ? <LockKeyhole className="h-3.5 w-3.5" /> : <X className="h-3.5 w-3.5" />}{privacy.data?.persistenceAllowed ? "الحفظ مفعل" : "الحفظ متوقف"}</span><Button onClick={() => { toast.success("انتهت الجلسة في واجهة العرض. يمكنك الآن مراجعة المحضر."); setLocation(minutesPath); }} className="h-10 rounded-xl bg-rose-500 px-3 text-xs font-bold text-white hover:bg-rose-600"><CircleStop className="ml-1.5 h-4 w-4" />إنهاء</Button></div></div></footer>
  </div>;
}
