...
function ControlButton({ label, active, onClick, children }: { label: string; active?: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} aria-label={label} className={`grid h-10 w-10 place-items-center rounded-xl border transition-colors ${active ? "border-indigo-200 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}>{children}</button>;
}
...
export default function Meeting() {
  ...
  const handleRecognizedText = useCallback(async (text: string) => {
    ...
    try {
      if (user) {
        const result = await translateText.mutateAsync({ text, sourceLanguage: "ar", targetLanguage: "en" });
        translatedText = result.translation;
      }
    } catch {
      toast.error("ظهر النص، لكن تعذر إكمال ترجمته الآلية هذه المرة.");
    }
    setTranscript(current => [...current, { id: Date.now(), speaker: user?.name || "أنت", initials: "أ", origin: text, translation: translatedText, time: "الآن", language: "العربية", tone: "bg-amber-100 text-amber-800" }]);
  ...
  }, [inviteCode, privacy.data?.persistenceAllowed, saveSegment, translateText, user, validInviteCode]);

  const liveSpeech = useLiveSpeech("ar-SA", handleRecognizedText);
  ...
  const sendMessage = async () => {
    ...
    if (translationOn && user) {
      try { translated = (await translateText.mutateAsync({ text: original, sourceLanguage: "ar", targetLanguage: "en" })).translation; }
