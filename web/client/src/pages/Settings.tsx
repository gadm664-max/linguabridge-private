...
import { BadgeCheck, BellRing, Database, Globe2, LockKeyhole, LogIn, MessageCircleMore, Save, ShieldCheck, Volume2 } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";

const LanguageOptions = () => <SelectContent><SelectItem value="ar">العربية</SelectItem><SelectItem value="en">English</SelectItem><SelectItem value="fr">Français</SelectItem><SelectItem value="es">Español</SelectItem></SelectContent>;
...
export default function Settings() {
  ...
  return <div className="min-h-screen bg-slate-50"><AppNavigation /><main className="mx-auto max-w-[920px] px-4 py-8 sm:px-7 sm:py-11">
    ...
    <div className="mt-8 space-y-5">
      ...
      <section className="rounded-[24px] border border-sky-100 bg-white p-5 sm:p-6"><span className="flex items-center gap-2 text-lg font-bold text-slate-900"><Database className="h-5 w-5 text-sky-600" />شفافية بياناتك</span><p className="mt-2 text-sm leading-6 text-slate-500">لا يجمع LinguaBridge بياناتك سرًا، ولا يبيع بيانات الحساب أو محتوى الاجتماعات أو رقم هاتفك. نحتفظ فقط بما يلزم لتشغيل الحساب والجلسة وفق اختياراتك.</p><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-3.5"><b className="text-xs text-slate-800">الحساب والتفضيلات</b><p className="mt-1.5 text-[11px] leading-5 text-slate-500">معرّف الدخول واللغات والصوت لإظهار تجربتك وحفظها في حسابك.</p></div><div className="rounded-2xl bg-indigo-50 p-3.5"><b className="text-xs text-indigo-900">نص الاجتماع والمحضر</b><p className="mt-1.5 text-[11px] leading-5 text-indigo-700">لا يُحفظان إلا بعد تحقق موافقات الاجتماع. يمكنك مراجعة السجل وطلب الحذف.</p></div><div className="rounded-2xl bg-emerald-50 p-3.5"><b className="text-xs text-emerald-900">التحسين</b><p className="mt-1.5 text-[11px] leading-5 text-emerald-700">أي قياس مستقبلي يكون مجمعًا وغير معرّف للهوية بعد إشعار واضح، بلا صوت خام أو رسائل.</p></div></div><div className="mt-4 rounded-xl border border-dashed border-sky-200 bg-sky-50/60 p-3 text-xs leading-6 text-sky-900"><b>حقوقك:</b> يمكنك معرفة الغرض من كل فئة بيانات، رفض التخزين غير الضروري، ومراجعة المحاضر من سجل الجلسات. لن نفعّل تحليلات أو إعلانات أو مشاركة مع طرف ثالث دون توضيح وموافقة مناسبة.</div></section>
