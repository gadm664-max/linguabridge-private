...
export default function Lobby() {
  ...
  const [voice, setVoice] = useState(defaultVoiceProfile);
  ...
  const createMeeting = trpc.meetings.create.useMutation({
    onSuccess: meeting => {
      ...
      setLocation(`/meeting/${meeting.inviteCode}?share=${invite ? "1" : "0"}`);
    },
  ...
  const enterMeeting = () => {
    if (!consent) {
      toast.error(lobbyReadinessCopy.consent.required);
      return;
    }
    if (!user) {
      ...
      <main className="mx-auto max-w-[1220px] px-4 py-8 sm:px-7 sm:py-11">
        ...
        <div className="grid gap-8 lg:grid-cols-[.92fr_1.08fr] lg:items-start">
          ...
          <section className="order-1 rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-[0_22px_54px_-36px_rgba(43,48,104,.35)] sm:p-8 lg:order-2"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold tracking-wide text-indigo-600">جلسة جديدة</p><h2 className="mt-1 text-2xl font-bold tracking-[-.045em] text-slate-900 sm:text-3xl">اختر كيف تريد أن تتحدث وتفهم</h2></div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-slate-950 text-white"><MonitorUp className="h-5 w-5" /></span></div><p className="mt-3 max-w-lg text-sm leading-6 text-slate-500">سيبقى النص الأصلي مرئيًا بجوار ترجمته، وستُعرض لك المحادثة بمحاذاة لغة العرض التي تختارها.</p>
            ...
            <div className="mt-8 rounded-2xl border border-slate-100 bg-slate-50 p-4"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white text-indigo-600 shadow-sm"><UserRoundPlus className="h-5 w-5" /></span><div><p className="text-sm font-bold text-slate-800">{lobbyReadinessCopy.invite.title}</p><p className="mt-0.5 text-xs text-slate-500">{getLobbyInviteGuidance(invite)}</p></div></div><button type="button" onClick={() => setInvite(v => !v)} className={`relative h-7 w-12 rounded-full transition-colors ${invite ? "bg-indigo-600" : "bg-slate-300"}`} aria-pressed={invite} aria-label="تفعيل أو إيقاف أدوات مشاركة الدعوة"><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow-sm transition-transform ${invite ? "right-1" : "right-6"}`} /></button></div></div>
            <label className="mt-7 flex cursor-pointer items-start gap-3 rounded-2xl border border-transparent p-2.5 hover:border-indigo-100 hover:bg-indigo-50/45"><Checkbox checked={consent} onCheckedChange={checked => setConsent(Boolean(checked))} className="mt-0.5" /><span><span className="text-sm font-bold text-slate-700">{lobbyReadinessCopy.consent.title}</span><span className="mt-1 block text-xs leading-5 text-slate-500">{lobbyReadinessCopy.consent.detail}</span></span></label>
