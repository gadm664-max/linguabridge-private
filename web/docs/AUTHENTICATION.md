...
تعتمد الجلسات على cookies `httpOnly` و`Secure` في HTTPS، وتُحافظ على `SameSite=None` بما يتوافق مع البنية الحالية. لا يثق الخادم في `userId` القادم من العميل عند إنشاء سجلات محمية؛ يتم اشتقاقه من الجلسة المصادق عليها.
